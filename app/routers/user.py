...
#     current_user.role = payload.mode  # active mode
#     db.commit()
#     db.refresh(current_user)
#     return {"message": f"Switched to {payload.mode} mode"}


# # PASSENGER SIGNUP (same as yours, but sets is_passenger True + active_mode passenger)
...
# # DRIVER ENABLE / SIGNUP (UPGRADES existing passenger if exists)
# @router.post("/driver")
# def create_or_enable_driver(driver: schemas.DriverCreate, db: Session = Depends(get_db)):
...
#     # check if user already exists (passenger account)
#     existing_user = db.query(models.User).filter(models.User.dsu_reg_id == driver.dsu_reg_id).first()
...
#         existing_user.is_passenger = True  # keep passenger enabled
#         existing_user.role = "driver"      # active mode becomes driver

#         # update driver-specific images (optional)
#         existing_user.nic_image_url = cnic_path
#         existing_user.live_image_url = live_path
#         existing_user.license_image_url = driver.license_image_url

#         existing_user.otp_code = otp_code
#         existing_user.otp_expiry = otp_expiry
#         # existing_user.is_verified = False  # if you want re-verify

#         db.flush()
#         user_obj = existing_user

#     else:
#         # ✅ create new user as driver+passenger
...
#     print(user.otp_expiry < datetime.utcnow())
